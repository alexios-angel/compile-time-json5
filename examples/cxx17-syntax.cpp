// Using ctjson with C++17 (no class-type template parameters): the
...
static_assert(ctjson::is_valid<text>);

constexpr auto doc = ctjson::parse<text>();

static_assert(doc.template get<project_key>() == std::string_view{"ctjson"});
